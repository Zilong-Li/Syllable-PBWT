...
void show_server_usage(string name)
{
    cout << "Syllable-Query: space-efficient long-match query algorithm.\n"
         ...
         << "\t-f,--fifo <FILE>                     File path to create named pipe for communication with "
            "clients\n"
         << "\t-i,--input_panel <FILE>              Path to VCF input file\n"
         << "\t-b,--bits <VALUE>                    Value of B (64 or 128; default: 128), the number of "
            "sites to be grouped into one syllable\n"
         << "\t                                     A higher B leads to stricter minimum query lengths but "
            "improved space efficiency.\n"
         << "\t-s,--save                            Save panel data for fast loading when rerunning this "
            "program with --load\n"
         << "\t                                     You will be told the save file upon creation.\n"
         << "\t-l,--load <FILE>                     Path to load file (must have been the save file of a "
            "previous successful run)\n"
         << "\t-g,--gen_map <FILE>                  Path to genetic map file, to be used when query length "
            "unit is cM\n"
         << "\t                                     Format: Sites described by one line each with genetic "
            "location as the last tab-delimited field.\n"
         << "\t                                             To accommodate header lines, lines with a "
            "non-numeric last field will be ignored.\n"
         << "Example: " << name << " -f myfifo -i sample.vcf -s\n"
         << "Usage note: The --save and --gen_map flags are optional. Exactly one of the flags --input_panel "
            "and --load should be used."
         << endl;
}
...
string show_client_usage()
{
    return "\nYou may enter queries using the following flags:\n"
           ...
           "Format of reported matches: Haplotypes will be referred to with the sample ID in the VCF file, "
           "followed by -0 or -1, denoting the first or second haplotype, respectively, of the individual. "
           "You will be told the location of the txt file containing one match reported per line, consisting "
           "of at least 4 tab-delimited fields $1 $2 $3 $4, indicating a match between query haplotype $1 "
           "and panel haplotype $2 over sites [$3, $4). If the query unit is cM or bps, there will be a "
           "fifth field indicating the length of the match in terms of the unit specified.";
}
...
void listen()
{
    ...
    if(input_file_set)
    {
        ...
        int code = sq.precompute((string)input_file);
        if(code == 1)
        {
    ...
    else
    {
        ...
        if(code == 3)
        {
            cerr << "The specified --bits value, " << B
                 << ", does not coincide with the one in the load file, " << sq.B << "." << endl;
            return;
        }
    ...
    while(true)
    {
        ...
        int code = sq.query(query_file.c_str(), output_file.c_str(), (int)ceil(L));
        ...
        {
            output(fdf, "The query length, " + to_string(L) + " " + unit
                            + ", is less than the minimum allowable query length, "
                            + to_string(unit == "sites" ? sq.minSiteL
                                        : unit == "cM"  ? sq.minGeneL
                                                        : sq.minPhysL)
                            + " " + unit + ".");
        }
        ...
        {
            output(fdf,
                   "Query file not in VCF format or has a different number of sites from the initial panel.");
        }
        else
        {
...
int main(int argc, char ** argv)
{
    ...
    if(B == 64)
        listen<unsigned long long>();
    else
        listen<unsigned __int128>();
