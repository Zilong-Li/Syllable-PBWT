...
template<class T>
int SyllableQuery<T>::precompute(std::string vcfpanel, std::string samples, std::string region)
{
...
template<class T>
vector<T> SyllableQuery<T>::encode_zg(const vector<int> & iz)
{
    vector<T> zg(n);
    for(int K = 0; K < N; K++)
    {
...
template<class T>
int SyllableQuery<T>::query(const vector<T> & zg, const int L, IntMapU & haplens, IntMapU & hapends)
{ // 1-length sites
    ...
    for(int k = l - 1, kp1 = l, t = 0, up_on = 0, dn_on = 0, no_beg = 0; k < n; k = kp1 - 1)
    {
        ...
        while(lo_p < hi_p)
        {
            ...
            if((h[xi][kp1] < h[xi][kp1 - l] ? MOD - h[xi][kp1 - l] + h[xi][kp1] : h[xi][kp1] - h[xi][kp1 - l])
               == zh)
            {
                hi_p = g_p;
            }
